...
function initDb() {
  ...
  const stmt = db.prepare<unknown[], {count: number}>('SELECT COUNT(*) AS count FROM users');
...
export async function getPosts(maxNumber?: number): Promise<Post[]> {
  ...
  const stmt = db.prepare<[number?], Post>(`
...
export async function storePost(post: Post) {
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?)`);
  await new Promise((resolve) => setTimeout(resolve, 1000));
  return stmt.run(post.image, post.title, post.content, post.userId);
}
